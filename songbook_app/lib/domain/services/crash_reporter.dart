...
class CrashReport {
  ...
  @override
  String toString() =>
      'CrashReport($fingerprint x$occurrences at ${route ?? '?'}: $message)';
...
class ConsoleCrashReporter extends CrashReporter {
  ...
  Future<void> report(CrashReport report) async {
    ...
    debugPrint('[crash] ${report.fingerprint}$repeats '
...
class ThrottledCrashReporter extends CrashReporter {
  ...
  Future<void> report(CrashReport report) async {
    try {
      final occurrences = _throttle.admit(report.fingerprint);
