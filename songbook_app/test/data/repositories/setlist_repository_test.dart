...
void main() {
  ...
  group('create', () {
    test('creates a named, empty setlist with an id', () async {
      ...
      expect(created.songNumbers, isEmpty);
  ...
  group('addSong', () {
    test('appends songs in order', () async {
      ...
      await repo.addSong(s.id, 42);
      await repo.addSong(s.id, 1);
      await repo.addSong(s.id, 151);

      expect(repo.getById(s.id)!.songNumbers, equals([42, 1, 151]));
    });
    ...
    test('is idempotent for an already-present song', () async {
      ...
      expect(repo.getById(s.id)!.songNumbers, equals([42]));
    });
  });
  ...
  group('removeSong', () {
    ...
    test('removing a missing song is a successful no-op', () async {
      ...
      final result = await repo.removeSong(s.id, 999);

      expect(result, isTrue);
      expect(repo.getById(s.id)!.songNumbers, equals([1]));
    });
  });
  ...
  group('reorderSongs', () {
    test('applies the exact requested order', () async {
      ...
      await repo.reorderSongs(s.id, [151, 1, 42]);

      expect(repo.getById(s.id)!.songNumbers, equals([151, 1, 42]));
    });
  });
  ...
  group('unknown id', () {
    test('mutators return false and do not throw', () async {
      ...
      expect(await repo.addSong('nope', 1), isFalse);
      expect(await repo.removeSong('nope', 1), isFalse);
      expect(await repo.reorderSongs('nope', [1]), isFalse);
    });
  });
  ...
  group('persistence across instances', () {
    test('a fresh repository over the same prefs reads setlists back', () async {
      ...
      await repo.addSong(s.id, 1);
      await repo.addSong(s.id, 42);
      await repo.reorderSongs(s.id, [42, 1]);
      ...
      expect(restored.songNumbers, equals([42, 1]));
    });
  });
}
